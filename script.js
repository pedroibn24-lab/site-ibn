...
async function carregarBlogPosts() {
  ...
  try {
    const resposta = await fetch('https://ibnegocios.com.br/cms_Dev/wp-json/wp/v2/posts?_embed&per_page=50');
...
async function carregarPosts() {
  ...
  try {
    const resposta = await fetch('https://ibnegocios.com.br/cms_Dev/wp-json/wp/v2/posts?_embed&per_page=3');
    ...
    posts.slice(0, 3).forEach((post, index) => {
      ...
      const titulo = post.title.rendered;
      const resumo = post.excerpt.rendered.replace(/<[^>]+>/g, '').substring(0, 120) + '...';
