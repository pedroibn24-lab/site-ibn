...
const _parallaxEls = () =>
  document.querySelectorAll(
    ".hero-bg-photo img, .section-bg-photo img, .photo-strip-img",
  );
...
function updateParallax() {
  ...
  requestAnimationFrame(() => {
    ...
    _parallaxEls().forEach((img) => {
      const section =
        img.parentElement?.closest("section") || img.parentElement;
...
function submitToWhatsApp(event) {
  ...
  window.open(
    `https://wa.me/551936450337?text=${encodeURIComponent(texto)}`,
    "_blank",
  );
  contactForm?.reset();
...
document
  .querySelector(".sc-arrow--prev")
  ?.addEventListener("click", () => changeService(-1));
document
  .querySelector(".sc-arrow--next")
  ?.addEventListener("click", () => changeService(1));
document.querySelectorAll(".sc-dot[data-index]").forEach((dot) => {
  dot.addEventListener("click", () =>
    goToService(parseInt(dot.dataset.index, 10)),
  );
});
document.querySelectorAll("a[data-service]").forEach((link) => {
  link.addEventListener("click", () =>
    goToService(parseInt(link.dataset.service, 10)),
  );
});
...
function normalizarUrlImagem(url) {
  ...
  return url
    .replace(
      /https?:\/\/dev\.ibnegocios\.com\.br\/blog\//g,
      "https://ibnegocios.com.br/cms_Dev/",
    )
    .replace(
      /https?:\/\/ibnegocios\.com\.br\/cmsDev\//g,
      "https://ibnegocios.com.br/cms_Dev/",
    );
}
...
function abrirArtigo(id) {
  ...
  const mainCat = terms.find(
    (t) =>
      t.taxonomy === "category" && t.name.toLowerCase() !== "sem categoria",
  );
  ...
  document.getElementById("artModalBody").innerHTML = sanitizeHtml(
...
document
  .getElementById("artModalClose")
  ?.addEventListener("click", fecharArtigo);
document
  .getElementById("artModalOverlay")
  ?.addEventListener("click", fecharArtigo);
...
async function carregarBlogPosts() {
  ...
  grid.innerHTML =
    '<div style="text-align:center;padding:3rem;color:#888">Carregando artigos...</div>';
  ...
  try {
    ...
    posts.forEach((post) => {
      ...
      terms.forEach((term) => {
        if (
          ...
          term.name.toLowerCase() !== "sem categoria"
        ) {
    ...
    posts.forEach((post, index) => {
      ...
      const resumo =
        post.excerpt.rendered.replace(/<[^>]+>/g, "").substring(0, 130) + "...";
      ...
      const allCatSlugs = terms
        .filter(
          (t) =>
            t.taxonomy === "category" &&
            t.name.toLowerCase() !== "sem categoria",
        )
        .map((t) => t.slug);
    ...
    grid.innerHTML =
      '<p style="text-align:center;padding:3rem;color:#666">Não foi possível carregar os artigos. Tente novamente mais tarde.</p>';
  }
}
  ...
  try {
    ...
    posts.slice(0, 3).forEach((post, index) => {
      ...
      card.addEventListener("keydown", (e) => {
        if (e.key === "Enter" || e.key === " ") abrirArtigo(post.id);
      });
...
function showTab(tab, btn) {
  document
    .querySelectorAll(".legal-section")
    .forEach((s) => s.classList.remove("active"));
  document
    .querySelectorAll(".legal-tab-btn")
    .forEach((b) => b.classList.remove("active"));
...
/* ============================================================
   ...
   ============================================================ */
document
  .querySelector(".wpp-bubble-close")
